            ...
            throws TorqueException
    {
        ...
        try 
        {
            ...
            if (criteria == null)
            {
                ...
                student.setAbsences(
                        getAbsences(resultSet, offset + 2));
                student.setFirstname(
                        getFirstname(resultSet, offset + 3));
                student.setName(
                        getName(resultSet, offset + 4));
                student.setGroupId(
                        getGroupId(resultSet, offset + 5));
            }
            else
            {
                ...
                for (Column column : columnsWithoutOffset)
                {
                    ...
                    else if (BaseStudentPeer.ABSENCES.getSqlExpression().equals(
                            column.getSqlExpression()))
                    {
                        student.setAbsences(
                            getAbsences(resultSet, totalOffset));
                        columnMapped = true;
                    }
                    else if (BaseStudentPeer.FIRSTNAME.getSqlExpression().equals(
                            column.getSqlExpression()))
                    {
                        student.setFirstname(
                    ...
                    else if (BaseStudentPeer.GROUP_ID.getSqlExpression().equals(
                            column.getSqlExpression()))
                    {
                        student.setGroupId(
         ...
         * Reads the value of the <code>columnIndex</code>'th column
         * in the <code>resultSet</code> so that it can be used to set
         * the field firstname in Student.
         *
         * @param dbObject the database object to fill, not null.
         * @param resultSet the ResultSet to read from, not null.
         * @param columnIndex the index in the resultSet which should be read.
